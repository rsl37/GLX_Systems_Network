...
if (
  nodeEnv &&
  ["development", "production", "staging", "test"].includes(nodeEnv)
) {
  console.log(`   ✅ NODE_ENV: Valid value "${nodeEnv}"`);
} else {
  console.log(
    `   ❌ NODE_ENV: Invalid or missing (current: "${nodeEnv || "NOT SET"}")`,
  );
}
...
if (jwtSecret) {
  if (jwtSecret.length >= 32) {
    console.log(
      `   ✅ JWT_SECRET: Valid length (${jwtSecret.length} characters)`,
    );
  } else {
    console.log(
...
if (clientOrigin) {
  try {
    ...
    if (isSecure) {
      console.log(
        `   ✅ CLIENT_ORIGIN: Valid URL with ${url.protocol} protocol`,
      );
    } else {
      console.log(
        `   ⚠️ CLIENT_ORIGIN: Should use HTTPS in production (current: ${url.protocol})`,
      );
    }
  } catch (error) {
