...
export function CountryCodeSelector({
  ...
  const selectedCountry =
    countries.find(country => country.dialCode === value) || popularCountries[0];
      ...
      <SelectContent className='w-[280px] max-h-[300px]'>
        ...
        {displayCountries.map((country, index) => {
          ...
          if (country.code === 'separator') {
            return <div key='separator' className='border-t my-1 mx-2' />;
          }
        ...
        {filteredCountries.length === 0 && searchQuery && (
          <div className='text-center py-6 text-sm text-gray-500'>No countries found</div>
        )}
      </SelectContent>
    </Select>
  );
