  ...
  async (req, res) => {
    try {
      ...
      if (phone) {
        ...
        const verificationCode = await generatePhoneVerificationToken(
          user.id,
          phone,
        );
        if (verificationCode) {
  ...
  try {
    const documentTypes = [
      { value: "passport", label: getDocumentTypeDisplayName("passport") },
      {
        ...
        label: getDocumentTypeDisplayName("drivers_license"),
      },
      {
        value: "national_id",
        label: getDocumentTypeDisplayName("national_id"),
      },
      {
        value: "residence_permit",
        label: getDocumentTypeDisplayName("residence_permit"),
      },
    ];
...
app.put(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (
        walletDisplayMode &&
        !["hidden", "public", "tip-button"].includes(walletDisplayMode)
      ) {
        return res.status(400).json({
          success: false,
          error: {
...
app.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    try {
      ...
      const offsetNumber =
        parseInt(offset as string) || (pageNumber - 1) * pageSize;
      ...
      const pagination = {
        ...
        links: {
          ...
          next: hasNext
            ? `/api/help-requests?page=${pageNumber + 1}&limit=${pageSize}`
            : null,
          prev: hasPrev
            ? `/api/help-requests?page=${pageNumber - 1}&limit=${pageSize}`
            : null,
...
app.post(
  "/api/help-requests/:id/offer-help",
  authenticateToken,
  async (req: AuthRequest, res) => {
    try {
      if (!req.userId || typeof req.userId !== 'number') {
...
app.get(
  "/api/chat/:helpRequestId/messages",
  authenticateToken,
  async (req: AuthRequest, res) => {
    try {
...
export async function startServer(port: number) {
  try {
    ...
    server.listen(port, () => {
      ...
      console.log(
        `🔌 Socket health: http://localhost:${port}/api/socket/health`,
      );
      console.log(
        `💰 Stablecoin API: http://localhost:${port}/api/stablecoin/status`,
      );
      console.log(
        `🛡️ Security Admin: http://localhost:${port}/api/admin/security/status`,
      );
      ...
      console.log(
        `🚀 Performance: Database indexes and connection optimizations active`,
      );
      console.log(
        `🧹 Socket management: Enhanced with connection cleanup and memory management`,
      );
    });
  } catch (err) {
